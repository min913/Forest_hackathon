...
def __find_closest_calories(way_effect, distance, height, weight):
    
    # 운동거리 차이 계산
    way_effect['운동거리_차이'] = abs(way_effect['운동거리'] - distance)
    # 운동거리 차이를 기준으로 정렬 후 상위 5개 행 선택
    closest_distances = way_effect.sort_values(by='운동거리_차이').head(5)
    
    # 키와 체중 차이 계산
    closest_distances['키_체중_차이'] = abs(closest_distances['키'] - height) + abs(closest_distances['체중'] - weight)
...
def run(distance, height, weight):
    return __find_closest_calories(forest_calorie_data, distance, height, weight)
